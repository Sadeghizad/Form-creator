    ...
    def validate(self, data):
        ...
        if data.get("is_private") and not data.get("password"):
            raise serializers.ValidationError(
                "A password is required for private processes."
            )

        if not data.get("order"):
            raise serializers.ValidationError(
                "Process must have at least one question."
            )
        ...
        for question_id in data["order"]:
            ...
            if question.user != user:
                raise serializers.ValidationError(
                    "You can only add questions you created."
                )
...
class QuestionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data["type"] == 2 or data["type"] == 3:
            if not data.get("order"):
                raise serializers.ValidationError(
                    "You must add options for test or checkbox questions."
                )
            user = self.context["request"].user
            for option_id in data["order"]:
                ...
                if option.user != user:
                    raise serializers.ValidationError(
                        "You can only add options you created."
                    )
...
class AnswerSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not select and not option and not text:
            raise serializers.ValidationError(
                "At least one of 'select', 'option', or 'text' must be provided."
            )
        return data
...
class FormSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        for process_id in data["order"]:
            ...
            if process.user != user and process.is_private:
                raise serializers.ValidationError(
                    "You can only add processes you own or that are public."
                )
        return data
    ...
    def validate_category(self, value):
        ...
        if value.user and value.user != user:
            raise serializers.ValidationError(
                "You are not allowed to add your form to this category."
            )
