...
class ProcessAPITests(UserTestCase):
    def setUp(self):
        ...
        self.question = Question.objects.create(
            user=self.user, text="Question 1", type=1
        )

    def test_create_process(self):
        response = self.client.post(
            reverse("processes-list"), {"order": [self.question.id]}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_order_field_not_empty(self):
        response = self.client.post(reverse("processes-list"), {"order": []})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class FormAPITests(UserTestCase):
    ...
    def test_private_form_requires_password(self):
        ...
        self.assertEqual(
            ...
            "A password is required for private forms.",
        )
...
class CategoryTests(APITestCase):
    def setUp(self):
        ...
        self.admin_user = User.objects.create_superuser(
            username="admin_user", email="admin@example.com", password="admin_pass"
        )
    ...
    def test_admin_can_create_public_category(self):
        ...
        response = self.client.post(
            reverse("public-category-list"), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_normal_user_creates_exclusive_category(self):
        ...
        response = self.client.post(
            reverse("exclusive-category-list"), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class FormProcessQuestionTests(TestCase):
    def setUp(self):
        ...
        self.form_linear = Form.objects.create(
        ...
        self.question1 = Question.objects.create(
            user=self.user, text="First question?", type=1, required=True
        )
        self.question2 = Question.objects.create(
            user=self.user, text="Second question?", type=1, required=True
        )
    ...
    def test_free_form_answer_submission(self):
        ...
        Answer.objects.create(
            ...
            question=self.question1,
            form=self.form_linear,
            text="Answer to question in free form",
        )
        Answer.objects.create(
            user=self.user,
            question=self.question2,
            form=self.form_linear,
            text="Answer to another question in free form",
        )
...
class FormProcessQuestionTests(JSONWebTokenTestCase):
    ...
    def test_answer_submission_order_linear_form(self):
        ...
        response = self.client.execute(
            ...
            variables={
                "input": {
                    "questionId": str(self.question2.id),
                    "text": "Attempt to answer out of order",
                    "formId": str(self.form_linear.id),
                }
            },
        )
        ...
        self.assertIn(
            "Answer previous questions in order first.", response.errors[0].message
        )
        ...
        response = self.client.execute(
            ...
            variables={
                "input": {
                    "questionId": str(self.question1.id),
                    "text": "Answer to question 1",
                    "formId": str(self.form_linear.id),
                }
            },
